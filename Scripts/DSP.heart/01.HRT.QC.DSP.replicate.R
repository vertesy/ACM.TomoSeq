...
TotalERCCCount = colSums(HeartSlices[grepv("^ERCC-",rownames(HeartSlices)), ])
...
if (Fig.S2.Sequencing.quality.control) {
  ...
  HQ_slices = which_names( (TotalTranscriptCount >= p$'MinTranscripts' & TotalERCCCount >= p$"min.ERCC") );l(HQ_slices)
  AdditionallyExcluded = setdiff(which_names( ! TotalERCCCount >= p$"min.ERCC"), which_names( ! TotalTranscriptCount >= p$'MinTranscripts' )); l(AdditionallyExcluded)

  plotLayout = rbind(c(1:2), c(3,3), c(4,4))
  ...
  if (FilterLQbyERCC) {
    ...
    wbarplot(log10(TotalERCCCount+1), hline = log10(p$"min.ERCC"), w=14, h=7, savefile = F, ylab="log10(ERCC count per section+1)", main="ERCC counts")
    pass = filter_HP(TotalERCCCount, threshold = p$"min.ERCC", plot.hist = F)
    HeartSlices = HeartSlices[ , pass]
  }
