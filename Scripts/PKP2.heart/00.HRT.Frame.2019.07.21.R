...
# InputFile = p0(InputDir, "Data/ACM.merged.ReadCounts.tsv.gz")
InputFile = p0(InputDir, "Data/ACM.merged.TranscriptCounts.tsv.gz")
OutDirOrig = OutDir = "~/Google_Drive/Avano/HRT/Analysis.2019.12.12.orig/"
setup_MarkdownReports(OutDir = OutDir, scriptname = "00.HRT.Frame.2019.07.21.R")
...
if (Section.Clustering) {
  ...
  x = pheatmap(slices_cor, cluster_cols = F, cutree_rows = 3, clustering_method = p$'clmethod.s.clust', silent = T)
  # CLust stat
  m$s$'clusterID' = hclust.getClusterID.row(pheatmapObject = x,k = 3); l(m$s$'clusterID')
  ...
  x = pheatmap(slices_cor, cluster_cols = F, cutree_rows = 3, clustering_method = p$'clmethod.s.clust', main = PlotName, col = colorRamps::matlab.like(100)
...
try.dev.off()
d1=color_branches(x$tree_row,k=3, col = sort(m$s$'cluster.colors'$categ))
